...
const RequestDetail = props => {
  ...
  // Activando la comunicacion, una vez que se iniciada la sesion. Lo que me garantiza que se abrio el socket
  useEffect(() => {
    if(session){
      ...
      socket.on('message:chat', message => {
        if(window.acmMessageId && window.acmMessageId !== message.id){
          window.acmMessageId = message.id;
          dispatch({ type: type.REQUEST_FORM, message: message })
        }

        if(!window.acmMessageId) {
          window.acmMessageId = message.id;
          dispatch({ type: type.REQUEST_FORM, message: message });
        }
      })
  ...
  useEffect(() => {
    ...
    request.hasOwnProperty('details') &&
      ...
      setDetails({
        ...
        messages: request.details.event.type == 'requestforattention' ? [{
          createdAt:request.details.event.createdAt,
          message: request.details.event.subject,
          id: 'initial-message',
          user:{
            code: request.details.event.irrigator.code,
            name: request.details.event.irrigator.name
          }
        },...request.details.messages] : request.details.messages,
